...
class HotSpotDetailVC: UIViewController, UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,CustomTableProtocol,UITextFieldDelegate {
    
    //variables
    var details = ["Title","Status","Feeder No.","Tree Species","Feeder Substation","Customers Count","Access to Tree","Environmental Conditions","OC Assigned","Comment"]
    var values : [String] = []
    ...
    override func viewDidLoad() {
        ...
        navigationBar.btnRight.addTarget(self, action: #selector(editHotSpot(_:)), for: .touchUpInside)
        
        
        //checking for work order id is available or not
        if ( hotSpotData.WorkOrderID != 0  ) {
        ...
        if ( hotSpotData.WorkOrderID != nil && hotSpotData.WorkOrderID == 0 &&  GetSaveUserDetailsFromUserDefault.getDetials()!.Type <= 2  ) {
    ...
    @objc private func editHotSpot(_ sender:UIButton){
        let vc = storyboard?.instantiateViewController(withIdentifier: "addHotSpot") as! AddHotSpotVC
        vc.hotSpotData = self.hotSpotData
        ...
        apiHandler.sendGetRequest(url: url,  completionHandler: { response,error in
            ...
            if((response["Status"] as! Int) != 0){
                ...
                for tempData in data{
                    if (tempData["UserId"] as! Int == self.hotSpotData.OCAssigned) {
    ...
    private func setupData(){
        ...
        if ( hotSpotData.WorkOrderID != 0 ) {
            ...
            let str = NSString(string: "Work order ID: \(hotSpotData.WorkOrderID)" )
            let theRange = str.range(of: "\(hotSpotData.WorkOrderID)")
            let theSecondRange = str.range(of:"Work order ID:")
            
            
            let myString:NSString = "Work order ID: \(hotSpotData.WorkOrderID)" as NSString
        ...
        values.append(hotSpotData.Title ?? "" )
        values.append((mcd.getRecordById(entityName:.Status, id: hotSpotData!.Status)["name"] as? String)!)
        values.append((mcd.getRecordById(entityName:.FeederList, id: hotSpotData!.FeederId)["name"] as? String)!)
        values.append(hotSpotData!.SpeciesName ?? "")
        values.append(hotSpotData!.FeederSubstation! == "0" ? "" : hotSpotData!.FeederSubstation!)
        let customerCount = hotSpotData!.FeederCustomerCount == 0 ? "":
        "\(hotSpotData!.FeederCustomerCount)"
        values.append(customerCount)
        values.append((mcd.getRecordById(entityName:.AccessToTree, id: hotSpotData!.AccessToTree)["name"] as? String)!)
        values.append(hotSpotData.EnvCondition!)
        let ocAssigned = hotSpotData!.OCAssigned == 0 ? "" : "\(hotSpotData!.OCAssigned)"
        values.append(ocAssigned)
        values.append(hotSpotData!.Comments!)
        
        txtStatus.setText(EntityName: .Status, recId: hotSpotData.Status)
        ...
        let historyTemp = hotSpotData.AuditList
    ...
    @objc func navigateAction(_ sender: UITapGestureRecognizer) {
        ...
        vc.objectLocationLat = hotSpotData.GeoLat
        vc.objectLocationLong = hotSpotData.GeoLong
        vc.objectType = "hzt"
        self.navigationController?.pushViewController(vc, animated: true)
        
        //        if (hotSpotData.GeoLong == 0.0 || hotSpotData.GeoLat == 0.0  ) {
        ...
        //        {
        //            let  url = "comgooglemaps://?saddr=&daddr=\(hotSpotData.GeoLat),\(hotSpotData.GeoLong)&directionsmode=driving"
        //            let finalURL = URL(string:url)!
        //          UIApplication.shared.open(finalURL, options: [:], completionHandler: nil)
        //        } else {
        //            // Navigate from one coordinate to another
        //            let url = "http://maps.apple.com/maps?saddr=&daddr=\(hotSpotData.GeoLat),\(hotSpotData.GeoLong)"
        //            let finalURL = URL(string: url)!
        //            UIApplication.shared.open(finalURL, options: [:], completionHandler: nil)
        //        }
    ...
    func viewImage(index:Int){
        ...
        let images = hotSpotData.HotSpotImages
    ...
    @objc func updateStatus(){
        ...
        let spotId = hotSpotData.HotSpotID
        
        var parameters : [String:AnyObject] = [String:AnyObject]()
        parameters["HotSpotId"] = spotId as AnyObject
        parameters["Status"] = status as AnyObject?
        parameters["UserId"] = userId as AnyObject
        
        
        apiHandler.sendPostRequestTypeSecond(url: apiUrl.updateHotSpotStatus,parameters:parameters, completionHandler: { response,error in
            ...
            if((response["Status"] as! Int) != 0){
                do{
                    ...
                    for tempData in data{
                        ...
                        if ( model.WorkOrderId == self.hotSpotData.WorkOrderID ) {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        vc.preSelectedId = hotSpotData.HotSpotID
        vc.workType = WorkType.HOTSPOT.rawValue
    }
}
