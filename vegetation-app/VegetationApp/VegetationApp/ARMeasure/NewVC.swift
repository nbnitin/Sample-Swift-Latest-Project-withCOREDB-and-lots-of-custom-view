...
//  AR-Ruler
//
//  Created by nitin bhatia on 1/5/18.
...
protocol ARMeasurePickedDelegate{
...
class NewVC: UIViewController, ARSCNViewDelegate,ARSessionDelegate {
    ...
    var delegate : ARMeasurePickedDelegate!
    var btnCapture : UIButton!
//    var arSceneView : ARSCNView = ARSCNView()
    ...
    @IBAction func btnCapture(_ sender: Any) {
        ...
        if ( (sender as! UIButton).title(for: .normal) == "Done" ) {
            self.delegate.diameterPicked(dia: self.dia)
            self.navigationController?.popViewController(animated: true)
            return
        }
